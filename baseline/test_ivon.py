...
if __name__ == '__main__':
    ...
    parser.add_argument("--last-checkpoint", type=str, required=True, help="Path to the last checkpoint")
    ...
    metrics_log_path = os.path.join(logdir, f"test-metrics-ivon.csv")
    ...
    with ConcurrentMetricsLogger(metrics_log_path) as logger:
        for metric, value in state.metrics.compute().items():
            logger.update('test', metric=metric, value=value, seed=args.seed, job_id=args.job_id)
